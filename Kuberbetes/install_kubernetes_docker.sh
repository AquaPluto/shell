...
HOSTS="
$MASTER1_IP    master1.$DOMAIN master1 kubeapi.$DOMAIN kubeapi
...
install_cri_dockerd () {
    ...
    sed -i '/^ExecStart/s#$# --network-plugin=cni --cni-bin-dir=/opt/cni/bin --cni-cache-dir=/var/lib/cni/cache --cni-conf-dir=/etc/cni/net.d --pod-infra-container-image registry.aliyuncs.com/google_containers/pause:'$PAUSE_VERSION#   /lib/systemd/system/cri-docker.service
...
install_kubeadm () {
    ...
    mkdir /etc/apt/keyrings	#ubuntu2004需要
